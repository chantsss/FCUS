...
from utils.metric_helpers import min_xde_K, bmin_xde_K, yaw_from_predictions, average_p, interpolate_trajectories, collisions_for_inter_dataset, miss_rate_k
...
class Evaluator:
    ...
    def _data_to_device(self, data):
        if "Joint" in self.model_config.model_type:
            ...
            instance_token = extras[1]
            sample_token = extras[2]
    ...
    def autobotego_evaluate(self):
        with torch.no_grad():
            ...
            for i, data in enumerate(self.val_loader):
                if i % 50 == 0:
                    print(i, "/", len(self.val_loader.dataset) // self.args.batch_size)
                ego_in, ego_out, agents_in, roads, safemap, ini_pose, instance_token, sample_token  = self._data_to_device(data)
                # permute safe map in ([bz, w, h, channel]) to bz, channel, w, h ([32, 3, 224, 224])
                # safemap = safemap.permute(0, 3, 1, 2)
            ...
            val_mode_probs = np.concatenate(val_mode_probs)
            val_dists = np.concatenate(val_dists)
            ...
            val_minfde_10 = min_xde_K(val_fde_losses, val_mode_probs, K=10)
            val_minfde_5 = min_xde_K(val_fde_losses, val_mode_probs, K=5)
            val_minfde_1 = min_xde_K(val_fde_losses, val_mode_probs, K=1)

            val_miss_rate_10_2 = miss_rate_k(val_dists, val_mode_probs, K=10)
            val_miss_rate_5_2 = miss_rate_k(val_dists, val_mode_probs, K=5)
            val_bminade_5 = bmin_xde_K(val_ade_losses, val_mode_probs, K=5)
            val_bminfde_5 = bmin_xde_K(val_fde_losses, val_mode_probs, K=5)
            a_p = average_p(val_mode_probs, K=self.model_config.num_modes)

            if "Gan" in self.model_config.model_type:
                val_violation_rate_mean = np.mean(val_violation_rates)
                print("minADE_{}:".format(self.model_config.num_modes), val_minade_c[0],
                    "minADE_10", val_minade_10[0], "minADE_5", val_minade_5[0], "minADE_1", val_minade_1[0],
                    "minFDE_{}:".format(self.model_config.num_modes), val_minfde_c[0], "minFDE_1:", val_minfde_1[0], \
                        "violation_rate", val_violation_rate_mean, "minFDE_5:", val_minfde_5[0],\
                        "val_miss_rate_10_2:", val_miss_rate_10_2, "val_miss_rate_5_2:", val_miss_rate_5_2,
                        "val_bminade_5:", val_bminade_5[0], "val_bminfde_5:", val_bminfde_5[0], "average_p:", a_p)
            else:
                val_violation_rate_mean = np.mean(val_violation_rates)
                print("minADE_{}:".format(self.model_config.num_modes), val_minade_c[0],
                    "minADE_10", val_minade_10[0], "minADE_5", val_minade_5[0],
                    "minFDE_{}:".format(self.model_config.num_modes), val_minfde_c[0], "minFDE_1:", val_minfde_1[0], "violation_rate", val_violation_rate_mean)
