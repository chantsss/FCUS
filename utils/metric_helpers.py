...
def average_p(probs, K):
    # best_ks = probs.argsort(axis=1)[:, -K:]
    # dummy_rows = np.expand_dims(np.arange(len(xdes)), 1)
    probs = np.sort(probs)
    return np.nanmean(probs, axis=1)

def bmin_xde_K(xdes, probs, K):
    best_ks = probs.argsort(axis=1)[:, -K:]
    dummy_rows = np.expand_dims(np.arange(len(xdes)), 1)
    new_xdes = xdes[dummy_rows, best_ks]
    perrs = np.power((1.0 - probs[dummy_rows, best_ks]), 2) * new_xdes
    return np.nanmean(np.sort(perrs), axis=0)

def miss_rate_k(xdes, probs, K, dist_thresh: float = 2):
    best_ks = probs.argsort(axis=1)[:, -K:]
    dummy_rows = np.expand_dims(np.arange(len(xdes)), 1)
    new_xdes = xdes[dummy_rows, best_ks]
    dist = np.max(new_xdes, axis=2)
    dist = np.min(dist, axis=1)
    m_r = np.sum((dist > dist_thresh)) / len(new_xdes)
    return m_r
