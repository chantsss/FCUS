...
from models.autobot_ego_gan import traj2patch, safety_checker
...
class AutoBotEgo(nn.Module):
    ...
    def forward(self, ego_in, agents_in, roads, ego_out, safemap, ini_pose):
        '''
        ...
        if self.training == True:
            ...
            violation_rate = self.compute_violation_rate(out_dists, ego_out, ini_pose, safemap)
    ...
    def compute_violation_rate(self, predictions, ground_truth: Union[Dict, torch.Tensor], safemap: torch.Tensor, \
        ...
        init_pose = ini_pose
        safe_mask = safemap.permute(0, 2, 3, 1)
        # sequence_length = traj_pred.shape[2]
